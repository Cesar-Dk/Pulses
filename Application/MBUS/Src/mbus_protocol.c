...
static char *mbus_json;
static char mbus_json_array[512];
...
//------------------------------------------------------------------------------
///
/// Encode time data (usable for type f = 4 bytes or type g = 2 bytes)
///
//------------------------------------------------------------------------------
void
mbus_data_tm_encode(time_t *t_time, u_char *t_data, size_t t_data_size)
{
	struct tm *t;
	...
	if (t_time && t_data)
    {
        ...
        else if (t_data_size == 4)                // Type F = Compound CP32: Date and Time
        {
        	...
        	t_data[0]  = t->tm_min  & 0x3F;
        	t_data[1]  = ((t->tm_hour & 0x1F)      | (t->tm_isdst << 7));
        	t_data[2]  = ((t->tm_mday & 0x1F)      | ((t->tm_year-100) & 0x7) << 5);
        	t_data[3]  = (((t->tm_mon + 1) & 0x1F) | (((t->tm_year-100) & 0xF8) << 1));
        }
        else if (t_data_size == 2)           // Type G: Compound CP16: Date
        {
        	t_data[0]  = 0;
        	...
        	t->tm_mday = 1;
        	t->tm_mon += 1;
        	t_data[0]  = ((t->tm_mday & 0x1F)      | ((t->tm_year-100) & 0x7) << 5);
        	t_data[1]  = (((t->tm_mon + 1) & 0x1F) | (((t->tm_year-100) & 0xF8) << 1));
        }
    }
}
#if 0
...
mbus_decode_manufacturer(u_char byte1, u_char byte2)
{
    ...
    m_id = mbus_data_int_decode((u_char *)m_str, 2);
...
size_t i64tostrn_a(signed long long x, char *s, size_t buff_size)
...
size_t u64tostrn_a(unsigned long long x, char *s, size_t buff_size)
{
...
//                                                (time.tm_year + 2000));
            ...
            case 0x04: // 4 byte (32 bit)
                ...
                if ( (vif == 0x6D) ||
                    ...
                    ((record->drh.vib.vif == 0xFD) && (vife == 0x70)) ||
					(((record->drh.vib.vif & 0x38) == 0x38) && (vife == 0x4E)) ||
					(((record->drh.vib.vif & 0x38) == 0x38) && (vife == 0x4F)))
                {
...
//                u64tostrn_a(val4, buff, sizeof(buff));
                i64tostrn_a(val4, buff, sizeof(buff));
...
//                u64tostrn_a(val4, buff, sizeof(buff));
            ...
            case 0x0B: // 6 digit BCD (24 bit)
            	...
            	if ( (record->drh.dib.dif & 0x30) == 0x30 ) {
//            		val = 0;
            		snprintf(buff, sizeof(buff), "%s", "ebbddd");
            	} else {
...
//                snprintf(buff, sizeof(buff), "%d", val4);
//                u64tostrn_a(val4, buff, sizeof(buff));
                i64tostrn_a(val4, buff, sizeof(buff));
            ...
            case 0x0F: // special functions
                ...
                mbus_data_bin_decode((u_char *)buff, record->data, record->data_len, sizeof(buff));
                break;

            case 0x0D: // variable length
                if (record->data_len <= 0xBF)
                {
                    mbus_data_str_decode((u_char *)buff, record->data, record->data_len);
                    break;
                }
                /* FALLTHROUGH */
    ...
    if (record)
    {
        switch (record->drh.dib.dif & MBUS_DATA_RECORD_DIF_MASK_FUNCTION)
        {
            case 0x00:
                ...
                snprintf(buff, sizeof(buff), "Ins");
                break;

            case 0x10:
                //snprintf(buff, sizeof(buff), "Maximum value");
            	snprintf(buff, sizeof(buff), "Max");
                break;

            case 0x20:
                //snprintf(buff, sizeof(buff), "Minimum value");
            	snprintf(buff, sizeof(buff), "Min");
                break;
...
//------------------------------------------------------------------------------
/// Parse the variable-length data of a M-Bus frame and
////Return a string containing an JSON representation of the M-BUS frame data.
//------------------------------------------------------------------------------
int
mbus_data_variable_parse_and_get_json(mbus_frame *frame, mbus_data_variable *data)
{
    mbus_data_record record;
    size_t i, j;
    size_t total_len = 0, buff_size = 256, data_len = 0;
    ...
    if (frame && data)
    {
        ...
        // first copy the variable data fixed header
        memcpy((void *)&(data->header), (void *)(frame->data), i);
        total_len += snprintf( &mbus_json_array[ total_len ], buff_size - total_len, "%s", mbus_data_variable_header_json( &( data->header )));
...
//                mbus_data_record_append(data, record);
            ...
            // append the record and move on to next one
#ifdef FALCON_PR6
            mbus_json = mbus_data_variable_json_from_record(data, &record, &data_len);
#elif defined(MX39)
            mbus_json = mbus_data_variable_json_dewa_from_record(data, &record, &data_len);
#elif defined(HYDRUS)
            mbus_json = mbus_data_variable_json_dewa_hydrus_from_record(data, &record, &data_len);
#elif defined(BADGER)
            mbus_json = mbus_data_variable_json_dewa_hydrus_from_record(data, &record, &data_len);
#endif
            if ( data_len != 0 ) {
            	memcpy(&mbus_json_array[ total_len ], mbus_json, data_len);
...
char * mbus_get_mbus_json_array( void )
{
	static char no_data[] = "_;";
	if( mbus_json_array == NULL )
		memcpy(mbus_json_array,no_data,2);

	return mbus_json_array;
}
//------------------------------------------------------------------------------
...
//            return mbus_data_variable_parse(frame, &(data->data_var));
            return mbus_data_variable_parse_and_get_json(frame, &(data->data_var));
        }
        else
        {
...
/// //------------------------------------------------------------------------------
/**
 * @fn int mbus_frame_pack(mbus_frame*, u_char*, size_t)
 * @brief Pack the M-bus frame into a binary string representation that can be sent on the bus.
 * 			The binary packet format is different for the different types of M-bus frames.
 * @param frame pointer to the @ref mbus_frame structure to write.
 * @param data pointer to buffer to write data to.
 * @param data_size length of the data to write.
 * @return offset
 * 			@arg -2
 * 			@arg -3
 * 			@arg -4
...
void mbus_str_json_encode( u_char *dst, const u_char *src, size_t max_len )
{
    size_t i, len;
...
//        mbus_str_json_encode( (u_char *)str_encoded, (u_char *)mbus_data_product_name( header ), sizeof( str_encoded ));
//        len += snprintf( &buff[ len ], sizeof( buff ) - len, "%s#", str_encoded );
        mbus_str_json_encode( (u_char *)str_encoded, (u_char *)mbus_data_variable_medium_lookup( header->medium ), sizeof( str_encoded ));
        len += snprintf( &buff[ len ], sizeof( buff ) - len, "%s#", str_encoded );
        len += snprintf( &buff[ len ], sizeof( buff ) - len, "%d#", header->access_no );
//        len += snprintf( &buff[ len ], sizeof( buff ) - len, "\"Status\":\"%.2Xh\"}", header->status );
        len += snprintf( &buff[ len ], sizeof( buff ) - len, "%d#", header->status );

//        len += snprintf( &buff[ len ], sizeof( buff ) - len, "%d#%d#", val, header->access_no );
        //len += snprintf( &buff[ len], sizeof( buff ) - len, "\"ID\":%d,", val );
        //len += snprintf( &buff[ len], sizeof( buff ) - len, "\"Mnf\":\"%s\",",
          //               mbus_decode_manufacturer( header->manufacturer[ 0 ], header->manufacturer[ 1 ]));
        //len += snprintf( &buff[ len ], sizeof( buff ) - len, "\"ver\":%d,", header->version );
        //mbus_str_json_encode( str_encoded, mbus_data_product_name( header ), sizeof( str_encoded ));
        //len += snprintf( &buff[ len ], sizeof( buff ) - len, "\"Name\":\"%s\",", str_encoded );
        ...
        //len += snprintf( &buff[ len ], sizeof( buff ) - len, "\"AccessN\":%d,", header->access_no );
        //len += snprintf( &buff[ len ], sizeof( buff ) - len, "\"Status\":\"%.2Xh\"}", header->status );
...
//------------------------------------------------------------------------------
#define DECIMAL(a,e)	((int)((fabsf((a-(float)((int)a))))*10^e))
...
{
    static char buff[ 40 ];//24
    char str_encoded[ 32 ];//16
    size_t len = 0;
    int8_t exp;

    if( record )
    {
        mbus_str_json_encode( (u_char *)str_encoded, (u_char *)mbus_data_record_value( record ), sizeof( str_encoded ));
//        exp = ( record->drh.vib.vif & 0x07 ) - 6;
//        len += snprintf( &buff[ len ], sizeof( buff ) - len, "%se%d", str_encoded, exp );
        if ( ((record->drh.vib.vif & 0x7F) != 0x6C)
           && ((record->drh.vib.vif & 0x7F) != 0x6D)
		   && (record->drh.vib.vif != 0xFD)
           && (record->drh.vib.vif != 0x26)
		   && (record->drh.vib.vif != 0xA5)
		   && (record->drh.vib.vife[0] != 0x4E)
		   && (record->drh.vib.vife[0] != 0x4F)
		   && (record->drh.vib.vife[0] != 0x5C)
		   && (record->drh.vib.vife[0] != 0x6C) ) {
        	if ((record->drh.vib.vif & 0x64) == 0x64) {// External Temp
        		exp = ( record->drh.vib.vif & 0x03 ) - 3;
        	}
        	else {
        		exp = ( record->drh.vib.vif & 0x07 ) - 6;
        	}
        ...
        else if (record->drh.vib.vif == 0x3A) {
        	exp = ( record->drh.vib.vif & 0x07 ) - 6;
        	len += snprintf( &buff[ len ], sizeof( buff ) - len, "%se%d", str_encoded, exp );
        }
        else {
...
char * mbus_data_variable_record_hydrus_json( mbus_data_record *record, int record_cnt, int frame_cnt, mbus_data_variable_header *header )
{
    static char buff[ 40 ];//24
    char str_encoded[ 32 ];//16
    size_t len = 0;
    ...
    if( record )
    {
        ...
        if ( ((record->drh.vib.vif & 0x7F)  != 0x6C)  //Time Point.
           && ((record->drh.vib.vif & 0x7F) != 0x6D) //Time Point.
		   && (record->drh.vib.vif != 0xFD)          //VIF Extension.
           && (record->drh.vib.vif != 0x26)          //Operating Time.
		   && (record->drh.vib.vif != 0xA4)          //Operating Time.
		   && (record->drh.vib.vif != 0xA5)          //Operating Time.
		   && (record->drh.vib.vif != 0xA6)          //Operating Time.
		   && (record->drh.vib.vif != 0x78)          //Fabrication No.
		   && ((record->drh.vib.vif & 0x7F) != 0x74) //Actuality Duration.
		   && (record->drh.vib.vife[0] != 0x4E)      //Date Time begin.
		   && (record->drh.vib.vife[0] != 0x4F)      //Date Time end.
		   && (record->drh.vib.vife[0] != 0x5C)      //Temperature.
		   && (record->drh.vib.vife[0] != 0x6C) ) {  //Battery.
        	if ((record->drh.vib.vif & 0x64) == 0x64) {// External Temp.
        		exp = ( record->drh.vib.vif & 0x03 ) - 3;
        	}
        	else {
...
char * mbus_data_variable_json( mbus_data_variable *data )
{
    ...
    if( data )
    {
		...
		for( record = data->record, i = 0; record; record = record->next, i++ )
        {
			// E001 0nnn - Volume && INS
			if((( record->drh.vib.vif & 0x78 ) == 0x10 ) &&
			   (( record->drh.dib.dif & MBUS_DATA_RECORD_DIF_MASK_FUNCTION ) == 0x00 ) &&
			   (  record->drh.dib.dif == 0x0C )) // & MBUS_DATA_RECORD_DIF_MASK_DATA
			{
				len += snprintf( &buff[ len ], buff_size - len, "%s#",
								 mbus_data_variable_record_json( record, i, -1, &( data->header )));
			}
			// E011 1nnn - Volume flow && INS
			if((( record->drh.vib.vif & 0x78 ) == 0x38 ) &&
			   (( record->drh.dib.dif & MBUS_DATA_RECORD_DIF_MASK_FUNCTION ) == 0x00 ) &&
			   (( record->drh.dib.dif == 0x02 ) || ( record->drh.dib.dif == 0x12 )))
			{
				len += snprintf( &buff[ len ], buff_size - len, "%s#",
								 mbus_data_variable_record_json( record, i, -1, &( data->header )));
			}
			...
			if((( record->drh.vib.vif & 0x7F ) == 0x6C ) &&
			   (( record->drh.dib.dif & MBUS_DATA_RECORD_DIF_MASK_FUNCTION ) == 0x00 ) &&
			   (  record->drh.dib.dif == 0xC2 )) // & MBUS_DATA_RECORD_DIF_MASK_DATA
			{
				len += snprintf( &buff[ len ], buff_size - len, "%s#",
								 mbus_data_variable_record_json( record, i, -1, &( data->header )));
			...
			// E110 110n - Time Point
			if((( record->drh.vib.vif & 0x7F ) == 0x13 ) &&
			   (( record->drh.dib.dif & MBUS_DATA_RECORD_DIF_MASK_FUNCTION ) == 0x00 ) &&
			   (( record->drh.dib.dif & 0x0F ) == 0x0C ) ) // & MBUS_DATA_RECORD_DIF_MASK_DATA
			{
				len += snprintf( &buff[ len ], buff_size - len, "%s#",
								 mbus_data_variable_record_json( record, i, -1, &( data->header )));
...
/// Generate JSON for variable-length data
//------------------------------------------------------------------------------
char * mbus_data_variable_json_from_record( mbus_data_variable *data, mbus_data_record *record, size_t *data_len )
{
    static char buff[ 256 ];
    size_t len = 0, buff_size = 256;
    int i = 0;
    ...
    if( data )
    {
			// E001 0nnn - Volume && INS
			if((( record->drh.vib.vif & 0x78 ) == 0x10 ) &&
			   (( record->drh.dib.dif & MBUS_DATA_RECORD_DIF_MASK_FUNCTION ) ==  MBUS_DATA_RECORD_DIF_MASK_INST ) &&
			   (  record->drh.dib.dif == 0x0C )) // & MBUS_DATA_RECORD_DIF_MASK_DATA
			{
				len += snprintf( &buff[ len ], buff_size - len, "%s#", mbus_data_variable_record_json( record, i, -1, &( data->header )));
			}
			// E011 1nnn - Volume flow && INS
			if((( record->drh.vib.vif & 0x78 ) == 0x38 ) &&
			   (( record->drh.dib.dif & MBUS_DATA_RECORD_DIF_MASK_FUNCTION ) ==  MBUS_DATA_RECORD_DIF_MASK_INST ) &&
...
//
//    if( data ) {
//		// E001 0nnn - Volume && INS 10^(nnn-6) m^3 nnn=0 (0x10).
//		if (((record->drh.vib.vif & 0x78) == 0x10)
//		 && ((record->drh.dib.dif & MBUS_DATA_RECORD_DIF_MASK_FUNCTION) == MBUS_DATA_RECORD_DIF_MASK_INST)
//		 && ((record->drh.dib.dif & MBUS_DATA_RECORD_DIF_MASK_DATA)     == MBUS_DATA_RECORD_DIF_MASK_TYPE_8DIGBCD)) {
//			len += snprintf(&buff[len], buff_size - len, "%s*", mbus_data_variable_record_json(record, i, -1, &(data->header)));
//		}
//		// E011 1nnn - Volume flow && INS 10^(nnn-6) nnn=0 (0x38).
//		if (((record->drh.vib.vif & 0x78) == 0x38)
//		 && ((record->drh.dib.dif & MBUS_DATA_RECORD_DIF_MASK_FUNCTION) == MBUS_DATA_RECORD_DIF_MASK_INST)
//		 && ((record->drh.dib.dif & MBUS_DATA_RECORD_DIF_MASK_DATA)     == MBUS_DATA_RECORD_DIF_MASK_TYPE_INT16)) {
//			len += snprintf(&buff[len], buff_size - len, "%s*", mbus_data_variable_record_json(record, i, -1, &(data->header)));
//		}
//		// E110 110n - Time Point date type G.
//		if (((record->drh.vib.vif & 0x7F) == 0x6C)
//		 && ((record->drh.dib.dif & MBUS_DATA_RECORD_DIF_MASK_FUNCTION) == MBUS_DATA_RECORD_DIF_MASK_INST)
//		 && ((record->drh.dib.dif & MBUS_DATA_RECORD_DIF_MASK_DATA)     == MBUS_DATA_RECORD_DIF_MASK_TYPE_INT16)) {
//			len += snprintf(&buff[len], buff_size - len, "%s*", mbus_data_variable_record_json(record, i, -1, &(data->header)));
//
//		}
//		// E001 0011 - Volume 10^(nnn-6) m^3 nnn=3 (0x13).
//		if (((record->drh.vib.vif & 0x7F) == 0x13)
//		 && ((record->drh.dib.dif & MBUS_DATA_RECORD_DIF_MASK_FUNCTION) == MBUS_DATA_RECORD_DIF_MASK_INST)
//		 && ((record->drh.dib.dif & MBUS_DATA_RECORD_DIF_MASK_DATA)     == MBUS_DATA_RECORD_DIF_MASK_TYPE_8DIGBCD)) {
//			len += snprintf(&buff[len], buff_size - len, "%s*", mbus_data_variable_record_json(record, i, -1, &(data->header)));
...
//char * mbus_data_variable_json_dewa_from_record( mbus_data_variable *data, mbus_data_record *record, size_t *data_len )
//{
//    static char buff[ 256 ];
//    size_t len = 0, buff_size = 256;
//    int i = 0;
...
//			// E001 0nnn - Volume && INS
//			if((( record->drh.vib.vif & 0x78 ) == 0x10 ) &&
//			   (( record->drh.vib.vif & 0x80 ) == 0x00 ) &&
//			   (( record->drh.dib.dif & MBUS_DATA_RECORD_DIF_MASK_FUNCTION ) ==  MBUS_DATA_RECORD_DIF_MASK_INST ) &&
//			   (( record->drh.dib.dif & MBUS_DATA_RECORD_DIF_MASK_DATA )     ==  MBUS_DATA_RECORD_DIF_MASK_TYPE_12DIGBCD ))
//			{
//				len += snprintf( &buff[ len ], buff_size - len, "%s#", mbus_data_variable_record_json( record, i, -1, &( data->header )));
//			}
//			// E011 1nnn - Volume flow && INS
...
//			}
//			// DEWA Serial Number.
//			if ((( record->drh.dib.dif & MBUS_DATA_RECORD_DIF_MASK_DATA) == MBUS_DATA_RECORD_DIF_MASK_TYPE_VAR_LENGTH ) &&
//			    ( record->drh.vib.vif     == 0xFD) &&
//			    ( record->drh.vib.vife[0] == 0x10))
//			{
//				len += snprintf( &buff[ len ], buff_size - len, "%s#", mbus_data_variable_record_json( record, i, -1, &( data->header )));
//			}
//			// E001 0nnn - Reverse Volume && INS, with extension byte.
//			if((( record->drh.vib.vif & 0x78 ) == 0x90 ) &&
//			   (  record->drh.vib.vife[0]      == 0x3C ) &&
//			   (( record->drh.dib.dif & MBUS_DATA_RECORD_DIF_MASK_FUNCTION ) ==  MBUS_DATA_RECORD_DIF_MASK_INST ) &&
//			   (( record->drh.dib.dif & MBUS_DATA_RECORD_DIF_MASK_DATA )     ==  MBUS_DATA_RECORD_DIF_MASK_TYPE_12DIGBCD ))
//			{
//				len += snprintf( &buff[ len ], buff_size - len, "%s#", mbus_data_variable_record_json( record, i, -1, &( data->header )));
//			}
//			if(( record->drh.vib.vif == 0xFD) && ( record->drh.vib.vife[0] == 0x28 ))
//			{
//				// Monthly values...
//			}
//			// E110 110n - Time Point
...
char * mbus_data_variable_json_dewa_hydrus_from_record( mbus_data_variable *data, mbus_data_record *record, size_t *data_len )
{
    static char buff[ 256 ];
    size_t len = 0, buff_size = 256;
    int i = 0;
    ...
    if( data )
    {
    	len += snprintf( &buff[ len ], buff_size - len, "%s#", mbus_data_variable_record_hydrus_json( record, i, -1, &( data->header )));
		*data_len = len;
        return buff;
    }
...
char * mbus_data_fixed_json( mbus_data_fixed *data )
{
    ...
    if( data )
    {
		...
		len += snprintf( &buff[ len ], buff_size - len, "    <SlaveInformation>\n");
        len += snprintf( &buff[ len ], buff_size - len, "        <Id>%d</Id>\n", (int) mbus_data_bcd_decode( data->id_bcd, 4 ));
        mbus_str_json_encode( (u_char *)str_encoded, (u_char *)mbus_data_fixed_medium( data ), sizeof( str_encoded ));
        len += snprintf( &buff[ len ], buff_size - len, "        <Medium>%s</Medium>\n", str_encoded );
        len += snprintf( &buff[ len ], buff_size - len, "        <AccessNumber>%d</AccessNumber>\n", data->tx_cnt );
        len += snprintf( &buff[ len ], buff_size - len, "        <Status>%.2X</Status>\n", data->status );
        len += snprintf( &buff[ len ], buff_size - len, "    </SlaveInformation>\n\n" );
        len += snprintf( &buff[ len ], buff_size - len, "    <DataRecord id=\"0\">\n" );
        mbus_str_json_encode((u_char *)str_encoded, (u_char *)mbus_data_fixed_function( data->status ), sizeof( str_encoded ));
        len += snprintf( &buff[ len ], buff_size - len, "        <Function>%s</Function>\n", str_encoded );
        mbus_str_json_encode( (u_char *)str_encoded, (u_char *)mbus_data_fixed_unit( data->cnt1_type ), sizeof( str_encoded ));
        len += snprintf( &buff[ len ], buff_size - len, "        <Unit>%s</Unit>\n", str_encoded );
        if(( data->status & MBUS_DATA_FIXED_STATUS_FORMAT_MASK ) == MBUS_DATA_FIXED_STATUS_FORMAT_BCD )
        {
            len += snprintf( &buff[ len ], buff_size - len, "        <Value>%d</Value>\n", (int) mbus_data_bcd_decode( data->cnt1_val, 4 ));
        ...
        mbus_str_json_encode( (u_char *)str_encoded, (u_char *)mbus_data_fixed_function( data->status ), sizeof( str_encoded ));
        len += snprintf( &buff[ len ], buff_size - len, "        <Function>%s</Function>\n", str_encoded );
        mbus_str_json_encode( (u_char *)str_encoded, (u_char *)mbus_data_fixed_unit( data->cnt2_type ), sizeof( str_encoded ));
        len += snprintf( &buff[ len ], buff_size - len, "        <Unit>%s</Unit>\n", str_encoded );
        if(( data->status & MBUS_DATA_FIXED_STATUS_FORMAT_MASK ) == MBUS_DATA_FIXED_STATUS_FORMAT_BCD )
        {
            len += snprintf( &buff[ len ], buff_size - len, "        <Value>%d</Value>\n", (int) mbus_data_bcd_decode( data->cnt2_val, 4 ));
...
char * mbus_data_error_json( int error )
{
    ...
    len += snprintf( &buff[ len ], buff_size - len, "<MBusData>\n\n" );
    len += snprintf( &buff[ len ], buff_size - len, "    <SlaveInformation>\n" );
    mbus_str_json_encode( (u_char *)str_encoded, (u_char *)mbus_data_error_lookup( error ), sizeof( str_encoded ));
    len += snprintf( &buff[ len ], buff_size - len, "        <Error>%s</Error>\n", str_encoded );
    len += snprintf( &buff[ len ], buff_size - len, "    </SlaveInformation>\n\n" );
    len += snprintf( &buff[ len ], buff_size - len, "</MBusData>\n");
...
{
    mbus_data_record *record;
    uint32_t len = sizeof(mbus_data_record);

    if ((record = (mbus_data_record *)malloc(len)) == NULL)
    {
        return NULL;
    }
...
//        snprintf(error_str, sizeof(error_str), "%s: frame or address arguments are NULL or invalid.", __PRETTY_FUNCTION__);
    ...
    frame->address             = MBUS_ADDRESS_NETWORK_LAYER;//253;  // for addressing secondary slaves
