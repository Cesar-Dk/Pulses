...
static int convert(dlmsVARIANT* item, DLMS_DATA_TYPE type)
{
    ...
    if (type == DLMS_DATA_TYPE_STRING)
    {
        ...
        if ( item->strVal == NULL )
        {
//        	item->strVal =  (gxByteBuffer*)&item_strVal;
//        	var_not_free = 1;
        }
        ...
        {
        	hlp_int64ToString(buff, 250, tmp.llVal, 1);//hlp_int64ToString(buff, 250, tmp.uiVal, 1);
            bb_addString(item->strVal, buff);
            item->vt = type;
            var_clear(&tmp);
            return DLMS_ERROR_CODE_OK;
