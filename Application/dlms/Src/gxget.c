...
int __attribute__((optimize("O0"))) getActivityCalendarDayProfileTable(gxArray* list, gxByteBuffer* ba)
{
    gxDayProfile* dp = (gxDayProfile*)malloc(sizeof(gxDayProfile));
    gxDayProfileAction* action = (gxDayProfileAction*)malloc(sizeof(gxDayProfileAction));;
    int ret, pos, pos2;
    memset(dp, 0, sizeof(gxDayProfile));
    memset(action, 0, sizeof(gxDayProfileAction));
...
{
    int ret = 0, pos;
    gxWeekProfile* wp = (gxWeekProfile*)malloc(sizeof(gxWeekProfile));
    memset(wp,0,sizeof(gxWeekProfile));
    //Add count
    if ((ret = cosem_setArray(ba, list->size)) != 0)
    {
        return ret;
...
{
    int pos, ret = 0;
    gxSeasonProfile* sp = (gxSeasonProfile*)malloc(sizeof(gxSeasonProfile));
