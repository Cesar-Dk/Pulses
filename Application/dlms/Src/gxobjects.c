...
int obj_clearPushObjectList(gxArray* buffer)
{
    ...
    for (pos = 0; pos != buffer->size; ++pos)
    {
        ...
        gxfree(kv->value);
        kv->value = NULL;/*RESET BUG*/
        gxfree(kv->key);
        kv->key = NULL;/*RESET BUG*/
        gxfree(kv);
        kv = NULL;/*RESET BUG*/
    }
...
int obj_clearDayProfileTable(gxArray* list)
{
    ...
    int pos, pos2;
    gxDayProfile it[1];
    gxDayProfileAction dp[1];
    memset(it, 0, sizeof(gxDayProfile));
    memset(dp, 0, sizeof(gxDayProfileAction));
    for (pos = 0; pos != list->size; ++pos)
    {
...
int obj_clearCertificateInfo(gxArray* arr)
{
    ...
    for (pos = 0; pos != arr->size; ++pos)
    {
        ...
        it->serialNumber = NULL;/*RESET BUG*/
        gxfree(it->issuer);
        it->issuer = NULL;/*RESET BUG*/
        gxfree(it->subject);
        it->subject = NULL;/*RESET BUG*/
        gxfree(it->subjectAltName);
        it->subjectAltName = NULL;/*RESET BUG*/
        gxfree(it);
        it = NULL;/*RESET BUG*/
    }
#endif //DLMS_IGNORE_MALLOC
...
void obj_clear(gxObject* object)
{
    ...
    if (object != NULL)
    {
        ...
        {
            if ((object->access->attributeAccessModes.data != NULL) && (object->access->methodAccessModes.data != NULL))
            {
