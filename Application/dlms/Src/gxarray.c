...
#define ARR_CAPACITY (100 * sizeof(void*))
uint32_t arr_data[ARR_CAPACITY];
...
void arr_clearKeyValuePair(gxArray* arr)
{
    ...
    if (arr->capacity != 0)
    {
        for (pos = 0; pos != arr->size; ++pos)
        {
            ...
            gxfree(k->key);
            k->key = NULL;/*RESET BUG*/
            gxfree(k->value);
            k->value = NULL;/*RESET BUG*/
            gxfree(k);
            k = NULL;/*RESET BUG*/
        }
        gxfree(arr->data);
