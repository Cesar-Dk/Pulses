...
//    bb_addString(&ba, "\n");
    bb_addString(&ba, ",");
//    bb_addString(&ba, obj_getUnitAsString(object->unit));
    bb_addIntAsString(&ba, object->unit);
    *buff = bb_toString(&ba);
    bb_clear(&ba);
    return ret;
}
...
//    bb_addString(&ba, "\nIndex: 3 Value: Scaler: ");
    bb_addIntAsString(&ba, hlp_getScaler(object->scaler));
//    bb_addString(&ba, " Unit: ");
//    bb_addString(&ba, obj_getUnitAsString(object->unit));
//    bb_addString(&ba, "Index: 2 Value: ");
    bb_addString(&ba, ",");
...
//    bb_addString(&ba, obj_getUnitAsString(object->unit));
    bb_addIntAsString(&ba, object->unit);
    bb_addString(&ba, ",");
//    if ( 1 == con_dlms_get_format_myd() )
    {
    	con_dlms_setDateFormat(&object->captureTime.value);
    }
...
int obj_ProfileGenericToString(gxProfileGeneric* object, char** buff)
{
    ...
    bb_addString(&ba, "]\nIndex: 3 Value: [");
    dlms_client_reset_load_profile_obis();
    obj_CaptureObjectsToString(&ba, &object->captureObjects);
    dlms_client_set_dlms_load_profile_num_obis(object->captureObjects.size);
//    dlms_client_set_load_profile_obis_block((char *)ba.data);
    bb_addString(&ba, "]\nIndex: 4 Value: ");
    bb_addIntAsString(&ba, object->capturePeriod);
    if ( ( 0 == udp_protocol_get_on_event_profile() ) && ( 0 == udp_protocol_get_on_billing_profile()))
    {
//    	dlms_client_set_dlms_load_profile_read_time(object->capturePeriod);
    	dlms_log_set_telegram_value_capture_period(object->capturePeriod);
    }
