...
// Constructor.
void __attribute__((optimize("O0"))) time_init(
    gxtime* time,
    uint16_t year,
    unsigned char month,
    unsigned char day,
    ...
    signed short devitation)
{
    ...
    if (time->value < offset)//719559//719561
    {
        time->value = 0;
    }
    else
    {
    	time->value -= offset;//time->value -= 719559L;//time->value -= 719561L;
    }
...
GXDLMS_DATE_FORMAT meter_date_format = GXDLMS_DATE_FORMAT_MDY;
int time_setMeterDateFormat(uint32_t _format)
{
	meter_date_format =(GXDLMS_DATE_FORMAT) _format;
	return 0;
}
int getDateFormat(GXDLMS_DATE_FORMAT* format, char* separator)
{
    ...
    return 0;
#else
    * format = GXDLMS_DATE_FORMAT_MDY;//meter_date_format;//
    *separator = '/';
    return 0;
#endif
}
    ...
    gxByteBuffer* ba)
{
    ...
    if ((time->skip & (DATETIME_SKIPS_YEAR | DATETIME_SKIPS_MONTH | DATETIME_SKIPS_DAY)) != (DATETIME_SKIPS_YEAR | DATETIME_SKIPS_MONTH | DATETIME_SKIPS_DAY))
    {
        ...
        default: //GXDLMS_DATE_FORMAT_MDY
        {
            ...
            else if ((time->skip & DATETIME_SKIPS_MONTH) == DATETIME_SKIPS_MONTH)
            {
            	empty = 0;
            	bb_addString(ba, GET_STR_FROM_EEPROM("**"));
            }
            ...
            else if ((time->skip & DATETIME_SKIPS_YEAR) == DATETIME_SKIPS_YEAR)
            {
            	empty = 0;
            	bb_addString(ba, GET_STR_FROM_EEPROM("/****"));
            }
