...
#endif
{
    ...
    if (!bb_isAttached(arr))
    {
        ...
        else
        {
            if (arr->capacity == 0)
            {
                if (getFS_totalAllocated() > 6700)
                {
                	LOGLIVE(LEVEL_1, "LOGLIVE> bytebuffer> WARNING!!!\r\nToo much allocated!!!!\r\n");
                 	return DLMS_ERROR_CODE_OUTOFMEMORY;//return DLMS_ERROR_CODE_OK;//
                }
    ...
    gxByteBuffer* arr)
{
    ...
    {
        memcpy(toString, arr->data, arr->size);
        *(toString + arr->size) = 0;
        if (arr->size > 500)
        {
        	asm("nop");
