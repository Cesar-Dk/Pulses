...
int hlp_bytesToHex2(const unsigned char* bytes, uint16_t count, char* buff, uint16_t size)
{
    ...
    if (count != 0)
    {
        ...
        {
            tmp = bytes[pos] & 0xFF;
            buff[pos * 2] = hexArray[tmp >> 4];//buff[pos * 3] = hexArray[tmp >> 4];
            buff[pos * 2 + 1] = hexArray[tmp & 0x0F];//buff[pos * 3 + 1] = hexArray[tmp & 0x0F];
//            buff[pos * 3 + 2] = ' ';
        }
        buff[(2 * count) - 1] = '\0';//buff[(3 * count) - 1] = '\0';
    }
    else
    {
        buff[0] = '\0';
...
int hlp_appendLogicalName(gxByteBuffer* bb, const unsigned char value[6])
{
    char ln[25];
    int ret = hlp_getLogicalNameToString(value, ln);
    dlms_client_set_load_profile_obis_n(ln);
    if (ret != 0)
    {
        return ret;
    }
